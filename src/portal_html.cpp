...
<div class="hero-tile-label">SoC temp</div>
<div class="hero-tile-value" id="heroTemp">—</div>
<div class="hero-tile-sub" id="heroTempSub">On-die · firmware polled</div>
</div>
<div class="hero-tile" id="heroTileWifi">
<div class="hero-tile-label">Wi‑Fi</div>
...
<div class="hero-tile-label">Tracking</div>
...
function heroWifiLabel(tier,rssi){
  if(tier<=0)return(rssi!=null&&Number(rssi)!==0)?('Weak · '+rssi+' dBm'):'No link';
  const labels=['','Fair','Good','Very good','Excellent'];
  return labels[tier]+(rssi!=null?' · '+rssi+' dBm':'');
}
...
function applyHero(j){
  ...
  const tb=j.battery_mv!=null,nstub=j.battery_state==='stub'&&!tb;
  if(hb){
    if(tb){hb.textContent=j.battery_mv+' mV';if(hts)hts.textContent='Voltage'}
    else if(nstub){hb.textContent='N/A';if(hts)hts.textContent='Sensor not installed'}
    else{hb.textContent='—';if(hts)hts.textContent=j.battery_state||''}
  }
  ...
  if(htt){
    ...
    if(htemps){
      if(ts==='warn')htemps.textContent='Warm · on-die sensor';
      else if(ts==='emergency'||ts==='thermal_hold')htemps.textContent='Thermal protection active';
      else htemps.textContent='On-die · firmware polled';
    }
  }
  const tier=ap?0:heroWifiTier(j.rssi);
  setHeroWifiBars(hwb,tier);
  if(hws){
    if(ap)hws.textContent='Setup AP · provisioning';
    else hws.textContent=heroWifiLabel(tier,j.rssi);
  }
  if(htr){
    if(j.thermal_hold){
      ...
      const im=j.imu_period_ms,pf=j.power_profile;
      const hz=im?Math.round(1000/im):'—';
      htr.textContent='~'+hz+' Hz';
      const pp=['Performance','Balanced','Battery saver'][pf]||'';
      if(htrk)htrk.textContent=pp?('IMU · '+pp):'';
    }
  }
}
